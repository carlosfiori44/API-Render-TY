...
export default class TaskController {
    ...
    static async search(req: Request, res: Response){
        ...
        if(!task) return res.status(404).json({ error: 'Task não encontrado' })
    ...
    static async update(req: Request, res: Response){
        ...
        //if(completed == undefined) return res.status(400).json({error: 'O completo é obrigatório!'});        

        if(completed === undefined && !title) return res.status(400).json({error: 'Preencha pelo menos um campo'})
